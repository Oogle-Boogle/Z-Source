...
public class PlayerHandler {
    ...
    public static void handleLogin(Player player) {
        ...
        if (!World.addPlayerUsername(player)) {
            LoginDecoder.sendReturnCode(player.getSession().getChannel(), LoginResponses.LOGIN_ACCOUNT_ONLINE);
            return;
        }
        player.getSession().setLoggedIn();
        ...
        player.getPacketSender().sendRichPresenceBigPictureText("https://zamron.us");
        ...
        player.getPacketSender()
                .sendMessage("<img=12><col=7838a3><shad=555>For any questions contact <col=dddddd><shad=111><img=3>Fatalsalmon <img=4>Oogle");
        player.getPacketSender()
                .sendMessage("<img=12><shad=20><col=b96900>Did you know? We reward players who report bugs!");
    ...
    public static boolean handleLogout(Player player, boolean fromUpdate, boolean fromQueue) {
        try {
            if (player.isMiniMe) {
                return true;
            }
            ...
            boolean exception = fromQueue || fromUpdate
                    || GameServer.isUpdating()
                    || player.getLogoutTimer().elapsed(600000);
    ...
    public static boolean handleLogout(Player player, boolean fromQueue) {
        try {
            if (player.isMiniMe) {
            ...
            boolean exception = GameServer.isUpdating() || fromQueue && player.getLogoutTimer().elapsed(600000);
                ...
                if (bank != null) {
                    ...
                    if (bankHolder != null) {
                        for (int i = 0; i < bankHolder.getBankTabs().length; i++) {
                            Bank newBank = new Bank(player);
                            for (Item item : bankHolder.getBank(i).getItems()) {
                                bank.add(item);
                            }
                            player.setBank(i, newBank);
                        }
