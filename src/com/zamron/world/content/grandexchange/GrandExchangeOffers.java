					...
					if(o.getType() == OfferType.SELLING && offer.getType() == OfferType.BUYING) {
						if(o.getPricePerItem() > offer.getPricePerItem() && o.getAmountFinished() < o.getAmount()) {
							...
							if(o.getFailAttempts() >= 3) {
								...
								if(p != null) {
									p.getPacketSender().sendMessage("").sendMessage("<img=12> <col=996633>Perhaps you should try lowering the price on your "+ItemDefinition.forId(item2).getName()+" offer").sendMessage("<col=996633>in the Grand Exchange. People are currently paying less for that item.");
									o.setFailAttempts(0);
								}
							}
							continue;
