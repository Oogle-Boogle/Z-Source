...
public class GrandExchange {
			...
			if(player.getSelectedGeSlot() >= 0 && player.getGrandExchangeSlots()[player.getSelectedGeSlot()].getState() == GrandExchangeSlotState.EMPTY) {
				...
				case 24738:
					...
					if(Item.sellable(player.getSelectedGeItem())) {
						...
						player.getPacketSender().sendMessage("<img=12> <col=996633>This item does not have a base price set.");
					}
					return true;
				case 24602:
				case 24702:
		...
		for(GrandExchangeOffer o : offers) {
			...
			if(o2.getUpdateState() != null) {
				...
				if(o2.getFailAttempts() >= 3) {
					player.getPacketSender().sendMessage("").sendMessage("<img=12> <col=996633>Perhaps you should try lowering the price on your "+ItemDefinition.forId(o2.getId()).getName()+" offer").sendMessage("<col=996633>in the Grand Exchange. People are currently paying less for that item.");
	...
	public static void setSelectedItem(Player player, int item) {
		...
		if(player.getInterfaceId() == BUY_INTERFACE) {
			...
			player.getPacketSender().sendMessage(cheapest == -1 ? "<img=12> <col=996633>Note: There are currently no players selling that item." : "<img=12> <col=996633>The cheapest offer found is currently selling at: "+Misc.insertCommasToNumber(""+cheapest+"")+" gp per item.");
			...
			player.getPacketSender().sendMessage(mostExpensive == -1 ? "<img=12> <col=996633>Note: There are currently no players buying that item." : "<img=12> <col=996633>The most expensive offer found is currently buying at: "+Misc.insertCommasToNumber(""+mostExpensive+"")+" gp per item."); 
