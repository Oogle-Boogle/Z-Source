...
import com.zamron.engine.task.impl.CeilingCollapseTask;
...
public class Barrows {
	...
	public static boolean handleObject(final Player player, GameObject object) {
		...
		case 10284:
			...
			if (player.getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[player.getMinigameAttributes().getBarrowsMinigameAttributes().getRandomCoffin()][1] == 0) {
				...
				if (Misc.getRandom(100) >= 98) {
					player.getInventory().addItem(CASH, 100000);
					World.sendMessageDiscord("@blu@[BARROWS] @bla@"+player.getUsername()+" has just received 100k 1B coins from the Barrows!");
				}
				if (Misc.getRandom(100) >= 84) {
					int b = randomBarrows();
					player.getInventory().add(b, 1);
					World.sendMessageDiscord("@blu@[BARROWS] @bla@"+player.getUsername()+" has just received "+ItemDefinition.forId(b).getName()+" from the Barrows!");
				}
	...
	public static int runes[] = {20260};

	static int CASH = 10835;

	public static int barrows[] = {13997, 11179};
