...
import static com.zamron.world.content.minigames.impl.PestControl.GameState.PLAYING;
import static com.zamron.world.content.minigames.impl.PestControl.GameState.WAITING;
...
public class PestControl {
    ...
    private static int playersInBoat = 0;

    private static final long lastDelay = 0;

    /**
     * @note Stores player and State
     */
    private static final Map<Player, GameState> playerMap = new HashMap<>();
    /*
     * Stores npcs
     */
    private static final List<NPC> npcList = new CopyOnWriteArrayList<>();

    /**
     * @return HashMap Value
     */
    public static GameState getState(Player player) {
    ...
    public static void boardBoat(Player p) {
        ...
        p.getPacketSender().sendString(15897, "@gre@Players Ready: " + totalPlayers);
    ...
    public static void leave(Player p, boolean fromList) {
        final GameState state = getState(p);
        if (state != null) {
            if (fromList) {
                playerMap.remove(p);
            }
    ...
    public static void sequence() {
        ...
        if (waitTimer <= 0) {
            if (!gameRunning)
                ...
                for (Entry<Player, GameState> entry : playerMap.entrySet()) {
        ...
        if (gameRunning) {
            ...
            if (Math.random() < 0.1)
                spawnRandomNPC();
            processNPCs();
            if (knight == null || knight.getConstitution() <= 0) {
                endGame(false);
                waitTimer = WAIT_TIMER;
...
//				p.getPacketSender().sendString(21009, "Pest Control Points: "+p.getPointsHandler().getCustompestcontrolpoints()+"");
    ...
    private static void startGame() {
        boolean startGame = !gameRunning && playersInBoat >= 1; //Players needed to start game
        if (startGame) {
            gameRunning = true;
            spawnMainNPCs();
        }
        for (Entry<Player, GameState> entry : playerMap.entrySet()) {
            ...
            if (WAITING.equals(state)) {
                Player p = entry.getKey();
                if (startGame) {
                    movePlayerToIsland(p);
                    playerMap.put(p, PLAYING);
                } else
                    p.getPacketSender().sendMessage("There must be at least 1 players in the boat before a game can start.");
            }
        }
    }
    ...
    private static void endGame(boolean won) {
        for (Entry<Player, GameState> entry : playerMap.entrySet()) {
            GameState state = entry.getValue();
            if (!PLAYING.equals(state)) continue;
            Player p = entry.getKey();
            leave(p, false);
        ...
        playerMap.clear();
        playersInBoat = 0; // use uppercase only for actual constants, which must be immutable (cannot be modified, AND if it's a class type its contents cannot be modified)
...
//		int knightHealth = 400000 - (PLAYERS_IN_BOAT * 5);
        ...
        npcList.add(knight);
        Collections.addAll(npcList, portals);
    }

    public static int getDefaultPortalConstitution() {
        return 15550000 + (playersInBoat * 190);
    }
    ...
    private static String getPortalText(int i) {
        return (portals[i] != null && (portals[i].getConstitution() > 0))
    ...
    private static void processNPCs() {
        for (NPC npc : npcList) {
            if (Location.PEST_CONTROL_GAME.equals(npc.getLocation()) && npc.getConstitution() > 0) {
                for (PestControlNPC value : PestControlNPC.values) {
                    if (npc.getId() >= value.lowestNPCID && npc.getId() <= value.highestNPCID) {
                        processPCNPC(npc, value);
                        break;
                    }
                }
            }
        }
        if (knight != null && knight.getConstitution() > 0 && Misc.randomBoolean(10)) {
            knight.forceChat(Misc.getRandom(KNIGHT_CHAT));
        }
    }
    ...
    private static void spawnRandomNPC() {
        for (NPC portal : portals) {
            if (portal != null && Math.random() > 0.5) {
                PestControlNPC luckiest = Misc.randomElement(PestControlNPC.values);
                if (luckiest != null) {
                    if (System.currentTimeMillis() - lastDelay >= 10000) {
                        npcList.add(spawnPCNPC(luckiest.getLowestNPCID() + ((int) (Math.random() * (luckiest.getHighestNPCID() - luckiest.getLowestNPCID()))), new Position(portal.getPosition().getX(), portal.getPosition().getY() + 5, 0), 300000));
                    }
                }
                if (luckiest != null) {
                    if (System.currentTimeMillis() - lastDelay >= 10000) {
                        npcList.add(spawnPCNPC(luckiest.getLowestNPCID() + ((int) (Math.random() * (luckiest.getHighestNPCID() - luckiest.getLowestNPCID()))), new Position(portal.getPosition().getX() + 5, portal.getPosition().getY(), 0), 300000));
                    }
                }
            }
        }
    ...
    private static void processSpinner(NPC npc) {
        ...
        for (NPC portal : portals) {
            if (portal != null && portal.getConstitution() > 0 && portal.getConstitution() > 0) {
                int distanceCandidate = distance(npc.getPosition().getX(), npc.getPosition().getY(), portal.getPosition().getX(), portal.getPosition().getY());
    ...
    public enum PestControlNPC {
        ...
        public static final PestControlNPC[] values = values();
    ...
    private static final NPC[] portals = new NPC[4];
    ...
    public static void buyFromShop(Player p, boolean item, int id, int amount, int cost) {
        ...
        final String comm = "Pest Control Points";
        if (!item) {
        ...
        p.getPacketSender().sendString(18729, "Pest Control Points: " + p.getPointsHandler().getCustompestcontrolpoints());
